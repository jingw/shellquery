...
def test_load_rows_ugly_name() -> None:
    connection = sqlite3.connect(":memory:")
...
def test_add_from_clause() -> None:
    ...
    assert (
        shellquery.add_from_clause("c1 limit 1", "table") == 'c1 FROM "table" limit 1'
    )
...
def _run_main_test(args: list[str], stdin_str: str = "") -> str:
...
def test_stdin() -> None:
    ...
    output = _run_main_test(["*"], "boring value")
    assert output == "boring\tvalue\n"


def test_unicode_stdin() -> None:
...
def test_header() -> None:
    ...
    output = _run_main_test(["'中' AS 文", "--output-header"], "a")
    assert output.splitlines() == ["文", "中"]

    # No rows to putput
    output = _run_main_test(["9 as colname", "-H"])
...
def test_re_split_randomly() -> None:
