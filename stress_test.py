...
def run_test(rows, max_columns, max_col_length, trials):
    print("rows={}, max_columns={}, max_col_length={}".format(
        rows, max_columns, max_col_length
    ))
    def rand_col():
        return ''.join(random.choice(string.ascii_letters) for _ in range(max_col_length))
    def rand_row():
    ...
    for i in range(trials):
        with tempfile.NamedTemporaryFile() as f:
            ...
            out = subprocess.check_output([
                sys.executable, shellquery.__file__,
